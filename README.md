...
In API mode the utility manages only the peers it could have added itself: outbound connections to public peers that are not listed in `-e`. Multicast (LAN) peers, peers at loopback, private (RFC1918) or unique-local addresses, inbound connections, the extra (`-e`) peers and peers with unrecognized URIs (e.g. `socks://`) are never removed. Peers are identified by their host (the port is disregarded): at most one managed connection per host, and an extra (`-e`) peer protects all connections to its host. If exactly `-n` managed peers are already connected and alive (with no down or duplicate leftovers to clean up), the utility exits without even downloading the list of public peers, so scheduled runs are cheap. Otherwise the managed peers are reconciled with the fresh list: those that are up and still among the `-n` best are kept, the rest are removed, and the missing best ones are added; if too few downloaded peers respond to ping, the already connected healthy peers keep the unfilled slots. If some peers with public URIs must never be replaced, list them in `-e`.
