...
В режиме API утилита управляет только теми пирами, которые могла добавить сама: исходящими соединениями с публичными пирами, не перечисленными в `-e`. Multicast-пиры (из локальной сети), пиры на loopback-, приватных (RFC1918) и unique-local адресах, входящие соединения, дополнительные (`-e`) пиры и пиры с нераспознанными URI (например, `socks://`) никогда не удаляются. Пиры различаются по хосту (порт не учитывается): не более одного управляемого соединения с одним хостом, а дополнительный (`-e`) пир защищает все соединения со своим хостом. Если уже подключено ровно `-n` живых управляемых пиров (и нет лишних — упавших или дублирующих — соединений), утилита завершается, даже не скачивая список публичных пиров, поэтому запуски по расписанию обходятся дёшево. Иначе управляемые пиры приводятся в соответствие со свежим списком: работающие и всё ещё входящие в число `-n` лучших остаются, остальные удаляются, а недостающие лучшие добавляются; если пинг прошли слишком мало скачанных пиров, незаполненные места сохраняют уже подключённые работающие пиры. Если какие-то пиры с публичными URI не должны заменяться никогда, укажите их в `-e`.
