...
Опции:
  ...
  -c, --config <FILE>   Путь к конфигурационному файлу Yggdrasil (используется при обновлении конфигурации с `-u`) [по-умолчанию: /run/yggdrasil/yggdrasil.conf или C:\ProgramData\Yggdrasil\yggdrasil.conf]
  ...
  -s, --socket <URI>    Адрес сокета admin API Yggdrasil [по-умолчанию: unix:///run/yggdrasil/yggdrasil.sock или tcp://localhost:9001]
  ...
  -r, --restart         Перезапускать сервис Yggdrasil (systemd или windows) (действует только вместе с `-u`)
...
В режиме API (`-a`) конфигурационный файл не читается: адрес админ-сокета берётся из опции `-s` (`--socket`) или её значения по-умолчанию. Конфигурационный файл нужен только для `-u`, поэтому `-a` работает даже на системах, где конфигурационный файл недоступен.
...
Добавление пиров (трёх) только через admin API — конфигурационный файл не требуется:
